...
1024x1536 transparent PNG, scenes 1024x1024 PNG. IDENTICAL LABEL: the composite prototype
(blank magenta label + our file laid in) was built, worked technically,
and he REJECTED it ("the label is no good this way — back to what we
had, just make the ask stricter; slight drift is bearable"). Removed.
Instead (committed, not deployed): the page sends the chosen label's id
(`frontId`), the route reads its words (engine labelWords) → the ask
carries EXACT WORDING, and in the scenes the REAL label rides first
(before the product shot) so scenes copy the label, not the shot's copy.
