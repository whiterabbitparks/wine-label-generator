...
/* /eval — THE EVALUATION LOOP (branch POPIKA_Back_To_Vector, 2026-09-18).

   One page, one job: the same six frozen briefs, dreamed through every
   style on every engine change, laid next to the reference boards, marked
   by the owner. Deliberately NOT a tab of /admin — admin is where rules
   are tuned; this is where we find out whether tuning did anything.
...
const STYLE_TITLE: Record<string, string> = { traditional: "Traditional", contemporary: "Contemporary", punk: "Punk" };
...
export default function EvalPage() {
  ...
  useEffect(() => { fetch("/api/admin/me").then((r) => r.json()).then((m) => { setAuthed(m.authenticated); if (m.authenticated) load(); }); }, [load]);
  ...
  if (!authed) return <LoginForm onSuccess={() => { setAuthed(true); load(); }} />;
        ...
        <div style={{ display: "flex", gap: 4, marginTop: 6, alignItems: "center" }}>
          ...
          {[1, 2, 3, 4, 5].map((s) => (
            <button key={s} onClick={() => rate(run, item, { score: rt?.score === s ? undefined : s })}
              style={{ ...S.btnGhost, padding: "2px 8px", fontSize: 12, background: rt?.score === s ? "#5a6b3b" : "transparent", color: rt?.score === s ? "#fff" : "#5a6b3b" }}>{s}</button>
          ))}
        </div>
  ...
  return (
    <main style={S.page}>
      <div style={{ display: "flex", alignItems: "baseline", gap: 16, flexWrap: "wrap" }}>
        ...
        <span style={{ fontSize: 12, color: "#6b6a60" }}>six frozen briefs · every style · every engine change · marked by the art director</span>
        <a href="/admin" style={{ marginLeft: "auto", fontSize: 12, color: "#5a6b3b" }}>admin →</a>
      </div>
      ...
      <div style={S.card}>
        ...
        {busy && <div style={{ marginTop: 10, height: 4, background: "#e2e1da", borderRadius: 2 }}><div style={{ width: `${busy.total ? (busy.done / busy.total) * 100 : 2}%`, height: 4, background: "#5a6b3b", borderRadius: 2, transition: "width 400ms" }} /></div>}
      </div>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {refs.map((r) => (
            ...
            <img key={r.id} src={r.url} alt={r.name} title={r.name}
              ...
              style={{ height: 96, border: pickRefFor ? "2px solid #5a6b3b" : "1px solid #ddd", cursor: pickRefFor ? "pointer" : "default", background: "#fff" }} />
          ))}
        ...
        return (
          <div key={brief.id} style={S.card}>
            <div style={{ display: "flex", gap: 12, alignItems: "baseline", flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 12, color: "#6b6a60" }}>{brief.width}×{brief.height} mm · {brief.data.wineColorName} · {brief.data.wineType}</span>
            </div>
