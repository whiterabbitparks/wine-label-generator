...
/* WHERE A CUSTOMER'S LABEL LIVES (round 84). The wizard holds the PNG for
   display; the SVG (live type, ~2 MB with its artwork embedded) and the
   painter's picture stay on disk under data/labels/<id>/ and the client
   carries only the id. The package step reads them back by id. */
...
export function saveLabel(l: { style: string; widthMm: number; heightMm: number; faces: string; ground: string; svg: string; png: string; art: string; prompt: string }): string {
...
export function readLabel(id: string): { meta: StoredLabel; svg: string; png: Buffer } | null {
